...
TST_N = 14
TST_PAD_N = 14 + 4
#TRN_RANGE = np.concatenate( (np.arange(0,7), np.arange(21,TRN_N) ),axis = 0)
...
Xtest[TRAIN_TRN_C] = Xtest[TRAIN_TRN_C].fillna(45)
...
#XGBR = xgb.XGBRegressor(max_depth = 3,learning_rate=0.03,n_estimators=1500)
...
for ind in range(14):
    ...
    time1 = time.time()
    GBR.fit(Xtrain_in.values[Good_ind],Y.values[Good_ind,ind] )
    y_true = Y.values[Good_ind,ind]
    y_pred = GBR.predict(Xtrain_in.values[Good_ind])
    time2 = time.time()    
    print(str(ind)+ '_error:' + str(abs_error(y_pred,y_true  ) ) + '__time:'+ str(time2 - time1) )
    error_list.append(abs_error(y_pred, y_true  ))
    Ytest = GBR.predict(Xtest_in.values)
...
#result_sub = result_sub.round()
#result_sub = result_sub.astype(np.int)


filename = 'GBDT_model.csv'
